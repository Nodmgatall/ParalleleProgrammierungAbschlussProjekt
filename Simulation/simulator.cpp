 ...
 *              (TODO: diesen schritt komplett abkoppeln und dann parallelisieren mit mergesort als
 * letzen schritt und pro 1 - n mit bubble_sort)
 ...
 *      - erhaelt dann in richtiger reinfolge die neu berechneten daten und fuegt sie wieder
 * zusammen
...
void Simulator::simulate_parallel() {
    ...
    int pro_id;
    MPI_Status status;
    Vec3<double>::create_mpi_type_vec3_double();
    Vec3<unsigned long>::create_mpi_type_vec3_ul();
    std::vector<int> chunk_sizes;
    unsigned long size;
    std::vector<std::tuple<unsigned long, unsigned long, unsigned long>> collison_data;
    ...
    while (current_iteration < m_number_of_iterations) {
        ...
        if (pro_id == 0) {
            printf("(%lu / %lu) %lu \r", (unsigned long)m_particles.get_time_simulated(),
...
#ifdef OUTPUT_DEBUG
            std::cout << pro_id << ": chunksize/sort/set_size done" << std::endl;
            std::cout << pro_id << ": starting sending object data(chuck size)" << std::endl;
#endif
            // Sending chunksizes
            MPI_Bcast(&chunk_sizes[0], number_of_procs - 1, MPI_INT, 0, MPI_COMM_WORLD);
            // Sending velocities vector
            m_particles.broadcast_object_data(size);
            ...
            std::vector<Vec3<unsigned long>> new_collision_data_vector;
            ...
            for (int i = 1; i < number_of_procs; i++) {
                ...
                unsigned long collision_data_size;
                ...
                std::vector<unsigned long> ul_buffer(new_size);

                MPI::COMM_WORLD.Recv(&v3_buffer[0], new_size, MPI_Vec3, i, tag);
                new_velocity_vector.insert(new_velocity_vector.end(), v3_buffer.begin(),
                                           v3_buffer.end());

                MPI::COMM_WORLD.Recv(&v3_buffer[0], new_size, MPI_Vec3, i, tag);
                new_pos_vector.insert(new_pos_vector.end(), v3_buffer.begin(), v3_buffer.end());

                MPI::COMM_WORLD.Recv(&d_buffer[0], new_size, MPI_DOUBLE, i, tag);
                ...
                MPI::COMM_WORLD.Recv(&ul_buffer[0], new_size, MPI_UNSIGNED_LONG, i, tag);
                new_id_vector.insert(new_id_vector.end(), ul_buffer.begin(), ul_buffer.end());
                MPI::COMM_WORLD.Recv(&collision_data_size, 1, MPI_UNSIGNED_LONG, i, tag);
                std::vector<Vec3<unsigned long>> collision_data_buffer(collision_data_size);
                MPI::COMM_WORLD.Recv(&collision_data_buffer[0], collision_data_size, MPI_Vec3_ul, i,
                                     tag);
                new_collision_data_vector.insert(new_collision_data_vector.end(),
                                                 collision_data_buffer.begin(),
                                                 collision_data_buffer.end());
            }
#ifdef OUTPUT_DEBUG
            ...
            m_particles.update_mass_vector(new_mass_vector);
            m_particles.update_radius_vector(new_radius_vector);
            m_particles.update_id_vector(new_id_vector);
            m_particles.update_collision_data_vector(new_collision_data_vector);
            ...
            if (current_iteration == m_number_of_iterations - 1) {
                m_particles.write_to_file(m_name_last_iteration_save_file,
                                          current_iteration + m_number_of_iterations_previous_run,
                                          std::ofstream::trunc | std::ofstream::binary);
            }
            // write_particle_archive_to_file();

            m_particles.write_to_file(m_name_output_file,
                                      current_iteration + m_number_of_iterations_previous_run,
            ...
            std::cout << "pro_id: " << pro_id
                      << "Size Velo: " << m_particles.get_velo_vector().size() << std::endl;
            std::cout << "pro_id: " << pro_id << "Size Pos: " << m_particles.get_pos_vector().size()
                      << std::endl;
            std::cout << "pro_id: " << pro_id
                      << "Size mass: " << m_particles.get_mass_vector().size() << std::endl;
            std::cout << "pro_id: " << pro_id << "Size id: " << m_particles.get_mass_vector().size()
                      << std::endl;
            for (unsigned long i = start; i < end; i++) {
                std::cout << "Pro: " << pro_id << " " << i << " "
                          << (m_particles.get_velo_vector()[i]).toString() << std::endl;
                std::cout << "Pro: " << pro_id << " " << i << " "
                          << (m_particles.get_pos_vector()[i]).toString() << std::endl;
                std::cout << "Pro: " << pro_id << " " << i << " "
                          << (m_particles.get_mass_vector()[i]) << std::endl;
            }
#endif

            m_particles.detect_collision(start + 2, end);
            unsigned long number_of_collisions = m_particles.get_collision_data().size();
            if (number_of_collisions > 0)
                std::cout << pro_id << " collisions: " << number_of_collisions << "end"
                          << std::endl;
            end = end - number_of_collisions;
            unsigned long new_size = chunk_sizes[pro_id - 1] - number_of_collisions;
            if (pro_id == 1) {
            ...
            MPI::COMM_WORLD.Send(&m_particles.get_radius_vector()[start], new_size, MPI_DOUBLE, 0,
                                 tag);
            MPI::COMM_WORLD.Send(&m_particles.get_mass_vector()[start], new_size, MPI_DOUBLE, 0,
                                 tag);

            MPI::COMM_WORLD.Send(&m_particles.get_id_vector()[start], new_size, MPI_UNSIGNED_LONG,
                                 0, tag);

            MPI::COMM_WORLD.Send(&number_of_collisions, 1, MPI_UNSIGNED_LONG, 0, tag);
            MPI::COMM_WORLD.Send(&m_particles.get_collision_data()[0], number_of_collisions,
                                 MPI_Vec3_ul, 0, tag);
...
void Simulator::set_up_simulation() {
    if (m_option_load_from_file) {
        ...
        // SUN
        m_particles.create_object(Vec3<double>(0, 0, 0), Vec3<double>(0, 0, 0),
                                  1.98855 * pow(10, 30), 696300000);
        // EARTH
        m_particles.create_object(Vec3<double>(AU, 0, 0), Vec3<double>(0, 29780, 0),
                                  5.972 * pow(10, 24), 6371000.0);
        ...
        for (unsigned long i = 0; i < m_number_of_particles - 2; i++) {
            m_particles.generate_stable_orbit_object(
                std::make_pair(AU * 1 / 2, AU),
                std::make_pair(5.972 * pow(10, 24), 5.972 * pow(10, 24) * 6),
                std::make_pair(100000, 6371000 * 15));
        }
...
void Simulator::get_options(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "help") == 0) {
        ...
        if (!is_all_digits(argv[1])) {
            std::cout << "ERROR: argument 2 [" << argv[1] << "] contains non digit parts"
                      << std::endl;
            exit(EXIT_FAILURE);
        }
...
void Simulator::usage() {
    std::cerr << "Usage: ./Simulation.x [option [file]] [particles] [iterations] [output file]"
              << std::endl;
...
void Simulator::print_help() {
    std::cout << "Usage: ./Simulation.x [option [file]] [particles] [iterations] [output file]"
              << std::endl;
    std::cout << std::endl;
